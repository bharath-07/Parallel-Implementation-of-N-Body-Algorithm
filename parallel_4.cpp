...
int main (int argc, char **argv) {
    ...
    if (fp == NULL) {
        ...
        for (int i = 0; i < n; i++) {
            int chunkId = (sched_getcpu()/8);
            insert (&root[chunkId], fx[i], fy[i], fz[i], fm[i]);
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                calculateNBody (fx[i], fy[i], fz[i], fm[i], &root[j], max_x, i, force_x, force_y, force_z, &force_locks[0]);
            }
        }
    }
