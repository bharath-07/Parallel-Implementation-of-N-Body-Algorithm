...
void insert (Node* start, double x, double y, double z, double m) {
    ...
    setOriginAndRange(originX, originY, originZ, rangeX, rangeY, rangeZ, octant);
...
int main (int argc, char **argv) {
    ...
    if (fp == NULL) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            root->child[i]->isDataNode = false;
            omp_init_lock(&root->child[i]->lock);
        }

        // Insert each of the n points into the tree
        #pragma omp parallel for num_threads(t)
        for (int i = 0; i < n; i++) {
            int octant = whichOctant (fx[i], fy[i], fz[i], 0.0, 0.0, 0.0);
            insert (root->child[octant], fx[i], fy[i], fz[i], fm[i]);
        }

        // Calculate the result for the n bodies
        for (int i = 0; i < n; i++) {
            calculateNBody (fx[i], fy[i], fz[i], fm[i], root, max_x, i, force_x, force_y, force_z, true);
        }
    }
